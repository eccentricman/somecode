...
public:
	...
	void setname(std::string meshname) {  name=meshname; };
	void computeTriangleNormals();
	void computeVertexNormals();
	void setxyz(int xx, int yy, int zz) { x = xx; y = yy; z = zz; }
	// 获取和设置物体的旋转平移变化
	...
	void generateBall(int num_division, float radius);
...
protected:
	...
	// 添加一个函数来生成草方块的索引
	void generateGrassBlockIndices();
	glm::vec3 translation;			// 物体的平移参数
	glm::vec3 rotation;				// 物体的旋转参数
	glm::vec3 scale;					// 物体的缩放参数
	glm::mat4 model ;

	glm::vec4 ambient= glm::vec4(0.25f, 0.25f, 0.25f, 1.0f);				// 环境光
	glm::vec4 diffuse= glm::vec4(0.5f, 0.5f, 0.5f, 1.0f);				// 漫反射
	glm::vec4 specular= glm::vec4(0.7f, 0.7f, 0.7f, 1.0f);				// 镜面反射
