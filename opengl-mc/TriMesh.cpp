...
glm::vec3 mc_cube[24] = {
	...
	// 左面
	cube_vertices[2], cube_vertices[0], cube_vertices[6], cube_vertices[4],
	// 右面
	cube_vertices[1], cube_vertices[3], cube_vertices[5], cube_vertices[7],
	// 后面
	cube_vertices[3], cube_vertices[2], cube_vertices[7], cube_vertices[6],
	// 前面
	cube_vertices[0], cube_vertices[1],cube_vertices[4], cube_vertices[5]
};
// 三角形的点
...
void TriMesh::setModelMatrix(glm::mat4 model)
{
	this->model = model;
}
...
void TriMesh::generateBlock()
{
	// 创建顶点前要先把那些vector清空
	cleanData();
	for (int i = 0; i < 24; i++)
	{
		vertex_positions.push_back(mc_cube[i]);
	...
	faces.push_back(vec3i(8, 9, 10));
	faces.push_back(vec3i(9, 11, 10));
	faces.push_back(vec3i(12, 13, 14));
	faces.push_back(vec3i(13, 15, 14));
	faces.push_back(vec3i(16, 17, 18));
	faces.push_back(vec3i(17, 19, 18));
	faces.push_back(vec3i(20, 21, 22));
	faces.push_back(vec3i(21, 23, 22));



	// 顶点纹理坐标，这里是每个面都用一个正方形图片的情况

	vertex_textures.push_back(log_textures[1]);//0
	vertex_textures.push_back(log_textures[0]);//1
	vertex_textures.push_back(log_textures[2]);//2
	vertex_textures.push_back(log_textures[3]);//3

	vertex_textures.push_back(log_textures[1]);//4
	vertex_textures.push_back(log_textures[0]);//5
	vertex_textures.push_back(log_textures[2]);//6
	vertex_textures.push_back(log_textures[3]);//7


	vertex_textures.push_back(log_textures[0]);//1
	vertex_textures.push_back(log_textures[5]);//3
	vertex_textures.push_back(log_textures[3]);//5
	vertex_textures.push_back(log_textures[4]);//7
...
void TriMesh::generateBall(int num_division, float radius)
{
	// 清空之前的模型数据
	cleanData();
	...
	for (int i = 0; i <= num_samples; ++i)
	{
		...
		for (int j = 0; j <= num_samples; ++j)
		{
			...
			// 添加顶点坐标
			vertex_positions.push_back(glm::vec3(x * radius, y * radius, z * radius));
			// 添加法向量
			vertex_normals.push_back(glm::vec3(x, y, z));
			// 添加颜色，这里采用法向量作为颜色
			vertex_colors.push_back(glm::vec3(x, y, z));
		}
	}
	...
	for (int i = 0; i < num_samples; ++i)
	{
		for (int j = 0; j < num_samples; ++j)
		{
			int first = i * (num_samples + 1) + j;
			int second = first + num_samples + 1;

			// 添加两个三角面片
			faces.push_back(vec3i(first, second, first + 1));
			faces.push_back(vec3i(second, second + 1, first + 1));
