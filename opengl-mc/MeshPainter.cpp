...
void MeshPainter::bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::string &texture_image, const std::string &vshader, const std::string &fshader){
    ...
    object.tLocation = glGetAttribLocation(object.program, "vTexture"); 
        ...
        glVertexAttribPointer(object.tLocation, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET((points.size() + colors.size() + normals.size()) * sizeof(glm::vec3))); 
...
void MeshPainter::addMesh( TriMesh* mesh,  const std::string &texture_image, const std::string &vshader, const std::string &fshader ){
...
void MeshPainter::removeMesh(TriMesh* mesh) {
    // 查找mesh在meshes中的索引
    auto it = std::find(meshes.begin(), meshes.end(), mesh);
    if (it != meshes.end()) {
        // 获取索引
        size_t index = std::distance(meshes.begin(), it);
        // 清理TriMesh数据和OpenGL资源
        mesh->cleanData();
        delete mesh;
        mesh = NULL;
        // 删除OpenGL资源
        glDeleteVertexArrays(1, &opengl_objects[index].vao);
        glDeleteBuffers(1, &opengl_objects[index].vbo);
        glDeleteProgram(opengl_objects[index].program);
        // 从容器中移除mesh和对应的OpenGL对象
        meshes.erase(it);
        opengl_objects.erase(opengl_objects.begin() + index);
    }
}
